        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const indicatorColor =
              color ??
              (item as { payload: { fill: string | undefined } }).payload
                .fill ??
              item.color;
            const separator =
              typeof separatorFormatter === "function"
                ? separatorFormatter(item, index)
                : separatorFormatter;
...
const ChartLegendContent = React.forwardRef<
  ...
  Omit<React.ComponentProps<"div">, "onClick"> &
    Pick<RechartsPrimitive.LegendProps, "payload" | "verticalAlign"> & {
      ...
      getDisabled?: (item: Payload) => boolean;
      labelFormatter?: (config: { item: Payload; key: string }) => string;
    }
>(
  (
    {
    ...
    return (
      ...
      >
        {payload.map((item) => {
          const key = `${nameKey ?? (item as { dataKey: string | undefined }).dataKey ?? "value"}`;
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
            ...
            >
              {itemConfig?.icon && !hideIcon ? (
                ...
                <div
                  ...
                  style={{
                    backgroundColor: disabled ? undefined : item.color,
                  }}
                />
              )}
              {labelFormatter
